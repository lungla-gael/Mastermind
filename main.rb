...
def human_codebreaker(human,computer)
    ...
    loop do
        ...
        feedback = []
        def matched?(color,human,computer)
            value = false
        ...
        human.secret_code.each do |position, color|
            if computer.secret_code.values.count(color) > 1 
                if color === computer.secret_code[position]
                    ...
                    feedback.unshift("white") if human.secret_code.values.first(position).count(color) === 1 && matched?(color,human,computer) == false
                    next
                end
            else
                if color === computer.secret_code[position]
                    ...
                    if matched?(color,human,computer) == false && computer.secret_code.values.include?(color)
...
def artificial_codebreaker(human,computer)
...
while is_valid_input === false
    ...
    when "codemaker"
        ...
        artificial_codebreaker(human,computer)
        if play_again?
            is_valid_input = false
        else
            is_valid_input = true            
    ...
    when "codebreaker"
        ...
        human_codebreaker(human,computer)
        if play_again?
            is_valid_input = false
        else
            is_valid_input = true            
